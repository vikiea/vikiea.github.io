...
- 应用包名迁移到 `io.github.vikiea.age`，并同步 gomobile engine 的 Java 包名。
- 加密、解密页面使用统一的玻璃卡片样式，操作历史 Tab 支持左右滑动切换。
- 密钥导入完成后显示短提示，减少重复打断。
...
当前公开版本是 `v4.0.0`，发布于 2026-05-28。本次版本将 Android 应用 ID 迁移到 `io.github.vikiea.age`，并同步 gomobile engine 包名；同时统一加密/解密页玻璃卡片样式，操作历史支持滑动切换，密钥导入完成后改为短提示。
